		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'file_list':
					...
					echo '<input class="upload_button button" type="button" value="Subir Archivo" />';
					echo '<p class="cmb_metabox_description">', $field['desc'], '</p>';
						$args = array(
				...
				case 'file':
					...
					echo '</div>'; 
				break;
				// Elementos personalizados. SFR
				case 'direccion_con_mapa':
					echo '<input class="cmb_text_medium" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					echo '<a id="geolocalizar" class="button-secondary" onclick="codeAddress()" style="margin-left:5px;">GEO Localizar</a>';
					break;
				case 'div_mapa':
					wp_enqueue_script('gMaps', 'http://maps.googleapis.com/maps/api/js?sensor=false&language=es');
					echo '<div id="map_canvas" style="width: 400px; height: 400px;"></div>'."\n";
					echo 	'<script type="text/javascript">
							jQuery(function($){				
								
								$(window).load(function() { 
									if ( $("#_cmb_infogeneral_latitud").val() != "" ){var latitud = $("#_cmb_infogeneral_latitud").val();}
									else{var latitud = -33.4166667;}
									if ( $("#_cmb_infogeneral_longitud").val() != "" ){var longitud = $("#_cmb_infogeneral_longitud").val();}
									else{var longitud = -70.55;}									
									initialize(new google.maps.LatLng(latitud, longitud));
								...
								function initialize(latlng) {
									...
									if (latlng.lat() != "-33.4166667" && latlng.lng() != "-70.55"){
										var marker = new google.maps.Marker({
											map: map, 
											position: latlng,
											draggable: true,
											icon: "'.WP_PLUGIN_URL.'/entorno/iconos-mapa/logo-almagro.png",
										});
										markersArray.push(marker);
										google.maps.event.addListener(marker, "dragend", function(event) {
											jQuery("#_cmb_infogeneral_latitud").val(event.latLng.lat());
											jQuery("#_cmb_infogeneral_longitud").val(event.latLng.lng());
								...
								function codeAddress() {
									clearOverlays();
									var address = jQuery("#_cmb_infogeneral_direccion").val();
									geocoder.geocode( { "address": address}, function(results, status) {
										if (status == google.maps.GeocoderStatus.OK) {
											jQuery("#_cmb_infogeneral_latitud").val(results[0].geometry.location.lat());
											jQuery("#_cmb_infogeneral_longitud").val(results[0].geometry.location.lng());			
